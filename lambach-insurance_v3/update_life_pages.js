...
const content = {
  term: {
    ...
    imageSrc: 'https://images.unsplash.com/photo-1542810634-71277d95dc8a?auto=format&fit=crop&q=80&w=1200',
  ...
  keyman: {
    ...
    imageSrc: 'https://images.unsplash.com/photo-1556761175-5973dc0f32b7?auto=format&fit=crop&q=80&w=1200',
...
function generatePage(data) {
  const allIcons = new Set(['ArrowRight', ...iconMap.features, ...iconMap.roles, ...iconMap.benefits]);
  ...
  return (
    <>
      ...
      <section className="py-24 bg-slate-50">
        <div className="layout-container">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold text-brand-marine mb-6">{ "${data.overviewHeading}" }</h2>
            <div className="space-y-4 mb-10 text-lg text-slate-600 leading-relaxed">
              ${data.overviewText.map(t => `<p>${t}</p>`).join('\n              ')}
            </div>
            
            <div className="mb-14">
              <h3 className="text-2xl font-bold text-brand-marine mb-6">{ "${data.featuresHeading}" }</h3>
              <ul className="grid md:grid-cols-2 gap-4">
                 ${data.features.map((f, i) => `
                   <li className="flex items-start gap-4 bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                     <${iconMap.features[i % iconMap.features.length]} className="w-6 h-6 text-primary flex-shrink-0 mt-1" />
                     <div>
                        <h4 className="text-slate-800 font-bold mb-1">${f.title}</h4>
      ...
      <section className="py-24 bg-white">
        <div className="layout-container">
          <div className="text-center mb-16 max-w-2xl mx-auto">
            ...
            <h2 className="text-3xl font-bold text-brand-marine mb-4">${data.roleHeading}</h2>
          </div>
            ...
            <div className="bg-blue-50 p-8 rounded-[2rem] flex flex-col group hover:shadow-xl transition-all duration-300 border border-transparent hover:border-blue-100">
              <div className="flex items-start justify-between mb-8 gap-4">
                ...
                <div className="w-16 h-16 flex-shrink-0 bg-white/60 rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
                  <${iconMap.roles[i % iconMap.roles.length]} className="w-7 h-7 text-blue-500" />
                </div>
              </div>
            </div>`).join('')}
          </div>
      ...
      <section className="py-24 bg-slate-50">
        <div className="layout-container">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold text-brand-marine mb-10 text-center">${data.benefitsHeading}</h2>
            <div className="grid sm:grid-cols-2 gap-6">
               ${data.benefits.map((b, i) => `
                <div className="flex items-start gap-4 p-6 bg-white rounded-2xl border border-slate-100 hover:shadow-md transition-shadow">
                  <div className="w-12 h-12 rounded-full bg-emerald-50 flex items-center justify-center flex-shrink-0">
                    <${iconMap.benefits[i % iconMap.benefits.length]} className="w-6 h-6 text-emerald-600" />
                  </div>
                  <div>
                    <h3 className="font-bold text-lg text-brand-marine mb-2">${b.title}</h3>
            ...
            <div className="mt-16 bg-white p-10 rounded-3xl shadow-lg border border-slate-100 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl"></div>
              <h3 className="text-2xl font-bold text-brand-marine mb-4 relative z-10">${data.closingHeading}</h3>
