...
const palettes = [
  { bg: 'bg-blue-50', border: 'hover:border-blue-100', text: 'text-blue-500' },
  { bg: 'bg-orange-50', border: 'hover:border-orange-100', text: 'text-orange-500' },
  { bg: 'bg-purple-50', border: 'hover:border-purple-100', text: 'text-purple-500' },
  { bg: 'bg-emerald-50', border: 'hover:border-emerald-100', text: 'text-emerald-500' },
  { bg: 'bg-rose-50', border: 'hover:border-rose-100', text: 'text-rose-500' },
  { bg: 'bg-fuchsia-50', border: 'hover:border-fuchsia-100', text: 'text-fuchsia-500' }
];
            ...
            <div className="w-full lg:w-7/12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ${data.features.map((f, i) => {
                  ...
                  return `
                  <div className="${p.bg} p-6 rounded-3xl border border-transparent shadow-sm group hover:shadow-md transition-all ${p.border}">
                    <div className="w-12 h-12 bg-white/60 rounded-xl flex items-center justify-center mb-4 shadow-sm group-hover:-translate-y-1 transition-transform">
                      <${iconMap.features[i % iconMap.features.length]} className="w-6 h-6 ${p.text}" />
                    </div>
                    <h4 className="font-bold text-lg text-brand-marine mb-2">${f.title}</h4>
        ...
        <div className="layout-container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ${data.roles.map((r, i) => {
              ...
              return `
            <div className="${p.bg} p-8 rounded-[2rem] flex flex-col group hover:shadow-xl transition-all duration-300 border border-transparent ${p.border}">
              <div className="flex items-start justify-between mb-8 gap-4">
                ...
                <div className="w-16 h-16 flex-shrink-0 bg-white/60 rounded-2xl flex items-center justify-center shadow-sm group-hover:-translate-y-1 transition-transform">
                  <${iconMap.roles[i % iconMap.roles.length]} className="w-7 h-7 ${p.text}" />
                </div>
              </div>
            </div>`;
            }).join('')}
        ...
        <div className="layout-container">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
             ${data.benefits.map((b, i) => {
               const p = palettes[(i + 4) % palettes.length];
               return `
              <div className="${p.bg} flex flex-col items-center text-center p-8 rounded-[2rem] border border-transparent hover:shadow-lg transition-all group ${p.border}">
                <div className="w-16 h-16 rounded-2xl bg-white/60 flex items-center justify-center mb-6 group-hover:-translate-y-1 transition-transform shadow-sm">
                  <${iconMap.benefits[i % iconMap.benefits.length]} className="w-8 h-8 ${p.text}" />
                </div>
                <h3 className="font-bold text-lg text-brand-marine mb-4">${b.title}</h3>
                <p className="text-slate-700 text-sm leading-relaxed">${b.desc}</p>
              </div>
             `;
             }).join('')}
          </div>
...
console.log('Successfully generated all color-enhanced pages.');
